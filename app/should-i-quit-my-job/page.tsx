...
export default function ShouldIQuitPage() {
  ...
  async function captureEmail() {
    ...
    await fetch("/api/capture-email", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: quizEmail, source: "should-i-quit", answers }) }).catch(() => {});
  ...
  const STATS = [
    { stat: "65%", label: "of people who quit impulsively regret the timing within 12 months (BLS, 2024)" },
    { stat: "40%", label: "who stayed and negotiated reported improvement within 6 months" },
    { stat: "3×", label: "more likely to be satisfied at 12 months if decision was data-driven vs gut feel" },
    { stat: "22%", label: "average salary increase for those who switched roles after proper analysis" },
  ];

  const FAQS = [
    { q: "Should I quit my job?", a: "We can't tell you that — and you shouldn't trust anyone who can without knowing your full situation. What we can do is show you what happened to people in situations similar to yours, and let you decide." },
    { q: "What if I've already decided to quit?", a: "Then the data is still useful — it helps you time it right, negotiate your exit, and avoid the mistakes people in your situation most commonly make." },
    { q: "What's the difference between Mirror and The Map?", a: "Mirror shows you your profile and risk score. The Map shows you all your paths forward — what each one looks like financially, emotionally, and career-wise, with real case data for each route." },
    { q: "Is this confidential?", a: "Yes. Your answers are used only to generate your report. We don't sell or share individual data." },
  ];
    ...
    <main className="min-h-screen bg-[#F5F6F8] text-[#0F1C3F]">
      <section className="mx-auto max-w-4xl px-6 py-16">
        <div className="rounded-3xl bg-white p-8 shadow-sm md:p-12">
          <div className="inline-flex items-center gap-2 rounded-full bg-amber-50 px-4 py-2 text-sm text-amber-700 font-semibold mb-4">
            Should I Quit?
          </div>
          <h1 className="text-3xl font-bold md:text-5xl leading-tight mb-4">
            Should you quit your job? Here's what 1,247 real cases show.
          </h1>
          <p className="text-lg opacity-75 mb-6 max-w-2xl">
            Most people make this decision on gut feel — and 65% regret the timing. We've analysed thousands of real career decisions to show you what actually happened to people in situations like yours.
          </p>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {STATS.map((s, i) => (
            <div key={i} className="rounded-2xl bg-white p-6 shadow-sm">
              <div className="text-3xl font-bold text-[#FF6A00] mb-2">{s.stat}</div>
      ...
      <section className="mx-auto max-w-4xl px-6 pb-10">
        <div className="rounded-3xl bg-white p-8 shadow-sm">
          <h2 className="text-2xl font-bold mb-6">The questions most people forget to ask</h2>
          <div className="space-y-4">
            {[
              { q: "Is it the job, the company, or the career?", a: "These have completely different solutions. Quitting the wrong thing doesn't fix the actual problem." },
              { q: "What does your financial runway actually look like?", a: "Most people overestimate how long they can sustain a gap and underestimate the cost of a rushed decision." },
              { q: "What have people in your exact situation done — and what happened?", a: "This is what Evaltree shows you. Real outcomes, not generic advice." },
              { q: "Are you leaving from something or towards something?", a: "People who leave towards a clear goal are 2× more likely to report satisfaction at 12 months." },
            ].map((item, i) => (
              <div key={i} className="rounded-2xl bg-[#F5F6F8] p-5">
                <div className="font-semibold text-sm mb-1">{item.q}</div>
                <div className="text-sm opacity-70">{item.a}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-4xl px-6 pb-10">
        <div className="rounded-3xl bg-white p-8 shadow-sm">
          <h2 className="text-2xl font-bold mb-2">Real cases — people who asked the same question</h2>
          <p className="text-sm opacity-60 mb-6">Anonymised. Details blurred. Outcomes real.</p>
          <div className="space-y-4">
            {[
              { role: "Marketing manager, 36", situation: "Asked 'should I quit?' every week for 8 months. Paralysed.", outcome: "Used The Map to see 4 clear paths. Chose internal transfer. Burnout resolved in 90 days.", tag: "Stayed — redirected", color: "border-emerald-200 bg-emerald-50 text-emerald-800" },
              { role: "Software engineer, 31", situation: "Toxic culture, underpaid by 30%, offer in hand from competitor", outcome: "Left. New role paid 28% more. At 12 months: higher satisfaction, better manager.", tag: "Left — thriving", color: "border-blue-200 bg-blue-50 text-blue-800" },
              { role: "Operations director, 48", situation: "Wanted to quit but kids in school, mortgage, limited runway", outcome: "Stayed 14 months, mapped exit strategy, left on own terms with 6-month runway.", tag: "Strategic exit", color: "border-amber-200 bg-amber-50 text-amber-800" },
            ].map((c, i) => (
      ...
      <section ref={quizRef} id="diagnostic" className="mx-auto max-w-4xl px-6 pb-10">
        {!quizStarted ? (
          <div className="rounded-3xl bg-[#0F1C3F] text-white p-8 shadow-sm text-center md:p-12">
            <h2 className="text-2xl font-bold mb-3">Free Career Diagnostic</h2>
            <p className="opacity-75 mb-6">7 questions. We match your situation against 1,247 real career decisions.</p>
          ...
          <div className="rounded-3xl bg-white p-8 shadow-sm md:p-10">
            ...
            {currentQ === 2 && !emailCaptured && (
              <div className="mt-6 rounded-2xl border border-[#FF6A00]/30 bg-[#FF6A00]/5 p-5">
                ...
                <p className="text-xs opacity-70 mb-3">We'll send your profile and similar case outcomes. No spam.</p>
          ...
          <div className="rounded-3xl bg-[#0F1C3F] text-white p-8 shadow-sm md:p-10">
            ...
            <p className="opacity-75 mb-6">Your profile shows a clear pattern. People who acted on data (not gut feel) were 3× more likely to report the right outcome at 12 months.</p>
